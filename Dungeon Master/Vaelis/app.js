...
function selected(spell) {
  const source = characterId === 'dungeon-master' ? state.selections : readJson('wizard-spells-selections-dungeon-master');
  return source[`${spell.owner}:${spell.id}`] === true;
}
...
async function load() {
  ...
  if (characterId !== 'dungeon-master') { state.spells.forEach(spell => { const id = `${spell.owner}:${spell.id}`; if (!(id in state.selections)) state.selections[id] = false; }); saveSelections(); }
...
function render() { const spells = filteredSpells(); elements.count.textContent = spells.length; elements.grid.innerHTML = spells.map(spell => `<article class="spell-card"><div class="card-art ${spell.image ? '' : 'no-art'}">${spell.image ? `<img src="${spell.image}" alt="" loading="lazy">` : '<span>✦</span>'}<b>${escapeHtml(spell.level)}</b>${characterId === 'dungeon-master' && view === 'equipped' ? starsFor(spell) : ''}</div><div class="card-content"><h2>${escapeHtml(spell.name)}</h2><p>${escapeHtml(spell.description)}</p><dl class="details">${spell.range ? `<div><dt>Alcance</dt><dd>${escapeHtml(spell.range)}</dd></div>` : ''}${spell.duration ? `<div><dt>Duración</dt><dd>${escapeHtml(spell.duration)}</dd></div>` : ''}${spell.dice ? `<div><dt>Efecto</dt><dd>${escapeHtml(spell.dice)}</dd></div>` : ''}</dl><div class="card-footer"><div class="tags">${characterId === 'dungeon-master' ? `<span class="owner-tag">${escapeHtml(characters[spell.owner].name)}</span>` : ''}</div>${characterId === 'dungeon-master' && view !== 'equipped' ? `<div class="choice"><button class="choice-button yes ${selected(spell) ? 'selected' : ''}" data-spell-owner="${spell.owner}" data-spell-id="${escapeHtml(spell.id)}" data-choice="yes">Sí</button><button class="choice-button no ${!selected(spell) ? 'selected' : ''}" data-spell-owner="${spell.owner}" data-spell-id="${escapeHtml(spell.id)}" data-choice="no">No</button></div>` : ''}</div></div></article>`).join(''); elements.empty.hidden = spells.length > 0; }
if (setupAccess()) {

...
function titleParts(value) {
  ...
  const parts = [text, text.split('(')[0], ...(text.match(/\(([^)]+)\)/g) || []).map(part => part.slice(1, -1))];
...
function imageFor(name, ownerId) {
  ...
  const file = files.find(item => {
    const fileParts = titleParts(item);
    return nameParts.some(namePart => fileParts.some(filePart => namePart === filePart || namePart.startsWith(filePart) || filePart.startsWith(namePart)));
  });
...
function render() { const spells = filteredSpells(); const currentSelections = state.spells.filter(selectedByUser).length; const countLink = document.querySelector('#nav-equipped-count'); if (countLink) countLink.textContent = currentSelections; elements.count.textContent = spells.length; elements.grid.innerHTML = spells.map(spell => { const choice = characterId === 'dungeon-master' ? approvedByDungeonMaster(spell) : selectedByUser(spell); const imageOnly = spell.owner === 'frederick' || spell.owner === 'minerva'; return `<article class="spell-card"><div class="card-art ${spell.image ? '' : 'no-art'}">${spell.image ? `<img src="${spell.image}" alt="" loading="lazy">` : '<span>✦</span>'}<b>${escapeHtml(spell.level)}</b>${characterId === 'dungeon-master' && view === 'equipped' ? starsFor(spell) : ''}</div><div class="card-content"><h2>${escapeHtml(spell.name)}</h2>${imageOnly ? '<p class="image-source-note">Información incluida en la imagen</p>' : `<p>${escapeHtml(spell.description)}</p><dl class="details">${spell.range ? `<div><dt>Alcance</dt><dd>${escapeHtml(spell.range)}</dd></div>` : ''}${spell.duration ? `<div><dt>Duración</dt><dd>${escapeHtml(spell.duration)}</dd></div>` : ''}${spell.dice ? `<div><dt>Efecto</dt><dd>${escapeHtml(spell.dice)}</dd></div>` : ''}</dl>`}<div class="card-footer"><div class="tags">${characterId === 'dungeon-master' ? `<span class="owner-tag">${escapeHtml(characters[spell.owner].name)}</span>` : ''}</div>${view !== 'equipped' ? `<div class="choice"><button class="choice-button yes ${choice ? 'selected' : ''}" data-spell-owner="${spell.owner}" data-spell-id="${escapeHtml(spell.id)}" data-choice="yes">Sí</button><button class="choice-button no ${!choice ? 'selected' : ''}" data-spell-owner="${spell.owner}" data-spell-id="${escapeHtml(spell.id)}" data-choice="no">No</button></div>` : ''}</div></div></article>`; }).join(''); elements.empty.hidden = spells.length > 0; }
if (setupAccess()) {
